...
public class SegmentTreeBuild {
    ...
    public SegmentTreeNode build(int start, int end) {
        ...
        System.out.println("expTreeNode " + root.start + " " + root.end);
        if (start != end) {
            int mid = start + (end - start) / 2;
            root.left = build(start, mid);
            System.out.println("left " + root.left.start + " " + root.left.end);
            root.right = build(mid + 1, end);
            System.out.println("right " + root.right.start + " " + root.right.end);
        }
        return root;
    }
