...
public class MemCache {
    private Map<Integer, Resource> client = null;
    ...
    public int get(int currentTime, int key) {
        ...
        if (res.expiredTime >= currentTime || res.expiredTime == -1) {
            return res.value;
        } else {
            return Integer.MAX_VALUE;
        }
    ...
    public void set(int currentTime, int key, int value, int timeToLive) {
        int expiredTime;
        if (timeToLive == 0) {
            expiredTime = -1;
        } else {
            expiredTime = currentTime + timeToLive - 1;
        }
        client.put(key, new Resource(value, expiredTime));
    }

    public void delete(int currentTime, int key) {
        if (!client.containsKey(key)) {
            return;
        }
        client.remove(key);
    ...
    public int increaseVal(int currentTime, int key, int delta) {
        if (!client.containsKey(key)) {
            return Integer.MAX_VALUE;
        }
        // set new timeval for resource
        Resource res = client.get(key);
        res.value += delta;
        // use existing get() method to return new resource
        return get(currentTime, key);
    }

    public int decreaseVal(int currentTime, int key, int delta) {
        if (!client.containsKey(key)) {
            return Integer.MAX_VALUE;
        }
...
class Resource {
    ...
    public Resource(int timeVal, int expiredTime) {
        this.value = timeVal;
